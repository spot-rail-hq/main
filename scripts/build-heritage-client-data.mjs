...
// Keyed by meta.slug, same stable identity as heritage-content.json — no
// segment-identity problem here. The one wrinkle: placement_confirmed is a
// field THIS generator writes (hardcoded false below), not an unknown field
// a human added — but flipping it to true is this file's own documented
// hand-edit workflow ("do not hand-edit except to confirm placements"). A
// bare inverted allowlist wouldn't protect it, since it's generator-owned by
// definition. So once an entry is confirmed, its whole placement (center,
// principal_station, placement_source, placement_confirmed) is frozen at
// the human-verified values instead of being recomputed — everything else
// on the entry (band, km, zoom, aliases, bbox, etc.) still refreshes
// normally from HERITAGE_META / the segment graph on every run.
const GENERATOR_OWNED_KEYS = new Set([
  'name', 'type', 'type_secondary', 'band', 'km', 'zoom',
  'placement_source', 'placement_confirmed', 'center', 'principal_station',
  'bbox', 'no_line', 'not_in_graph', 'needs_coordinates', 'aliases',
]);
const PLACEMENT_KEYS = ['center', 'principal_station', 'placement_source', 'placement_confirmed'];
...
function foreignFields(obj) {
  const out = {};
  for (const [k, v] of Object.entries(obj)) if (!GENERATOR_OWNED_KEYS.has(k)) out[k] = v;
  return out;
}

// Aborts (writing nothing) if any confirmed placement or unknown hand-added
// field would be silently dropped by this write.
function assertNoDataLoss(existing, fresh) {
  if (!existing) return;
  const lost = [];
  for (const [slug, oldEntry] of Object.entries(existing)) {
    if (slug === '_notes') continue;
    const newEntry = fresh[slug];
    if (oldEntry.placement_confirmed === true && !(newEntry && newEntry.placement_confirmed === true)) {
      lost.push({ slug, field: 'placement_confirmed', old: true, note: 'confirmed placement would be reset' });
    }
    const oldForeign = foreignFields(oldEntry);
    for (const [k, v] of Object.entries(oldForeign)) {
      if (v === undefined || v === null || v === '') continue;
      if (!newEntry || JSON.stringify(newEntry[k]) !== JSON.stringify(v)) {
        lost.push({ slug, field: k, old: v });
      }
    }
  }
  if (lost.length) {
    console.error(`\n✗ DATA LOSS DETECTED — ${lost.length} field(s) would be silently dropped or reset:`);
    for (const l of lost) console.error(`  ${l.slug}.${l.field}: ${JSON.stringify(l.old)}${l.note ? ` — ${l.note}` : ''}`);
    console.error(`\nRefusing to write ${OUT_PATH}.`);
    throw new Error('heritage-railways.json data loss guard tripped — see diagnostic above');
  }
}
...
// ─── merge in confirmed placements + any hand-added fields, then guard ────
const existing = loadExisting();
if (existing) {
  for (const [slug, entry] of Object.entries(out)) {
    const old = existing[slug];
    if (!old) continue;
    if (old.placement_confirmed === true) {
      for (const k of PLACEMENT_KEYS) {
        if (old[k] !== undefined) entry[k] = old[k]; else delete entry[k];
      }
    }
    Object.assign(entry, foreignFields(old));
  }
}
...
assertNoDataLoss(existing, out);
writeFileSync(OUT_PATH, JSON.stringify(payload, null, 2) + '\n');
if (existing) {
  const confirmedCount = Object.values(out).filter((e) => e.placement_confirmed === true).length;
  console.log(`Preserved ${confirmedCount} confirmed placement(s) from the previous output.`);
}
...
{
  const confirmed = Object.values(out).filter((e) => e.placement_confirmed === true).length;
  console.log(confirmed
    ? `${confirmed} of ${Object.keys(out).length} placements confirmed; the rest are placement_confirmed:false — review required.`
    : 'ALL placements are placement_confirmed:false — review required.');
}
